...
| Этап | Что | Статус |
|------|-----|--------|
| 0 | Документация и архитектурные решения | ✅ закрыт |
| 1 | Базовая инфраструктура (Django + Docker + core) | ✅ закрыт |
| 2 | Аутентификация (accounts, User+Profile, login/profile) | ✅ закрыт |
| 3 | Рабочие группы (модели, права, дерево, CRUD) | ✅ закрыт |
| 4 | Задачи (модели, статусы, история, повторение) | ✅ закрыт |
| 5 | Чаты (модели, polling, страница, мьюты) | ✅ закрыт |
| 6 | Уведомления (модель, троттлинг, интеграция) | ✅ закрыт |
| 7 | Файлы (Attachment, защищённое скачивание, валидация) | ✅ закрыт |
| 8 | Dashboard (срез, счётчики) | ✅ закрыт |
| 9 | Финал (демо-данные, автотесты, smoke-проверка, финальные правки) | ✅ закрыт |

Все 9 этапов закрыты. ВКР завершена. Проект продолжается как pet-project.
...
Готово: `apps/dashboard/` — только views + templates, без моделей. View `/` заменяет
заглушку `HomeView` из `apps/core`.

Блоки:
- **Мои задачи** — Bootstrap-вкладки «Назначены мне» / «Назначил я»; бейджи статуса,
  приоритета, группы; ссылка на полный список.
- **Мои группы** — список активных `WorkGroupMembership` с ролью и родительской группой.
- **Мои чаты** — список активных `ChatMembership`; красный индикатор (●) на чатах
  с непрочитанными уведомлениями `chat_new_message` (через `Exists`-subquery).
- **Уведомления** — последние 8 непрочитанных; клик → `notifications:go` → объект.

Дополнительно: исправлены метки типов чатов — `chat_type_label` property в модели
возвращает «Личный» / «Чат команды» / «Чат внутренней команды» / «Операционный»
(вместо «Личный» / «Группа» / «Групповой»).

---

## Этап 9 — Финал ✅

Commit: `feat: stage 9 — demo data, automated tests, smoke test fixes`.

**Блок 9.1 — Демо-данные** ✅

Management-команда `python manage.py create_demo` с флагом `--reset`.
Создаёт: 7 пользователей (admin, 2 headworker, 4 worker), 2 родительские +
3 дочерние группы, 7 задач в разных статусах, сообщения в чатах рабочих групп,
1 личный + 1 операционный чат. Флаг `--reset` очищает все данные кроме суперпользователя
и пересоздаёт сцену. Гайд встроен в docstring команды.
Реализовано в `apps/core/management/commands/create_demo.py`.

**Блок 9.2 — Smoke-проверка и правки** ✅

Ручная проверка 8 MVP-сценариев из `CLAUDE_PROJECT_CONTEXT.md` раздел 7.
По итогам исправлено:
- ghost-алерты над чатом (коллизия имён `messages` с Django messages framework →
  переменная контекста переименована в `chat_messages`);
- индикатор непрочитанных (●) добавлен на страницу списка чатов (`Exists`-subquery
  по `Notification`, аналогично dashboard);
- Django Admin: убран пустой раздел «Группы» (`auth.Group` unregister),
  добавлен favicon и заголовок «Администрирование 4К» через шаблон
  `templates/admin/base_site.html`;
- `ChatMembership.verbose_name_plural` → «Членство в чатах»;
- кнопка «Админ» в навигации для staff-пользователей.

**Блок 9.3 — Автотесты** ✅

84 unit-теста, покрытие 61% (измерено через `coverage`).
Тесты расположены в `apps/<app>/tests/test_services.py` и `test_permissions.py`
по шести модулям: tasks, workgroups, chats, notifications, attachments, core.
Общая база: `apps/core/tests/base.py` (хелпер `make_user`).
Конфигурация coverage: `.coveragerc` (исключены миграции, тесты, management-команды).
